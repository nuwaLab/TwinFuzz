...
def niqe(inputImgData):
    ...
    # The absolute path you used to store the niqe working directory
...
# Call cal_niqe_score to calculate the corresponding niqe score for any given input_image.
def cal_niqe_score(input_image):
    
    # Convert the MNIST image(28*28) to a larger size(e.g., 224*224)
    image = Image.fromarray(input_image)
    resized_image = np.array(image.resize((224,224), Image.BICUBIC))
    # Then Calculate score
    niqe_score = niqe(resized_image)
    # Return the final niqe score
    return niqe_score
