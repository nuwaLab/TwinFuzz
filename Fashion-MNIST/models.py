from tensorflow import keras

def LeNet5(input_shape=(28, 28, 1)):
    input_tensor = keras.layers.Input(shape=input_shape)
  
    x = keras.layers.Convolution2D(6, (5, 5), activation='relu', padding='same', name='block1_conv1')(input_tensor)
    x = keras.layers.MaxPooling2D(pool_size= (2, 2), name='block1_pool1')(x)

    x = keras.layers.Convolution2D(16, (5, 5), activation='relu', padding='same', name='block2_conv1')(x)
    x = keras.layers.MaxPooling2D(pool_size=(2, 2), name='block2_pool2')(x)
    
    x = keras.layers.Flatten(name='flatten')(x)
    x = keras.layers.Dense(120, activation='relu', name='fc1')(x)
    x = keras.layers.Dense(84, activation='relu', name='fc2')(x)
    x = keras.layers.Dense(10, name='before_softmax')(x)
    x = keras.layers.Activation('softmax', name='predictions')(x)
    
    return keras.models.Model(input_tensor, x)

def LeNet4(input_shape = (28,28,1)):
    input_tensor = keras.layers.Input(shape = input_shape)

    # The first convolutional block
    x = keras.layers.Conv2D(6,(5,5), activation = 'relu', padding = 'same', name = 'block1_conv1')(input_tensor)
    x = keras.layers.MaxPool2D(pool_size = (2,2), name = 'block_pool1')(x)

    # The second convolutional block
    x = keras.layers.Conv2D(16,(5,5), activation = 'relu', padding = 'same', name = 'block2_conv2')(x)
    x = keras.layers.MaxPooling2D(pool_size=(2,2), name = 'block2_pool2')(x)

    # Then, we flatten the output and dense layers
    x = keras.layers.Flatten(name = 'flatten')(x)
    x = keras.layers.Dense(84, activation = 'relu', name = "fc1")(x)
    x = keras.layers.Dense(10, name = 'before_softmax')(x)
    x = keras.layers.Activation('softmax', name = 'prediction')(x)

    return keras.models.Model(input_tensor, x)

